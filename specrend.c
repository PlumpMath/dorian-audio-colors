...
void norm_rgb_1(double *r, double *g, double *b)
{
#define Max(a, b)   (((a) > (b)) ? (a) : (b))
    double greatest = Max(*r, Max(*g, *b));
...
void wave_to_xyz(double wave, double *x, double *y, double *z){
  *x = xFit_1931(wave);
  *y = yFit_1931(wave);
  *z = zFit_1931(wave);
}
...
{
    int i;
    double t, w, x, y, z, r, g, b;
    struct colourSystem *cs = &CIEsystem;

    printf("wave              x      y      z       R     G     B\n");
    printf("-----------    ------ ------ ------   ----- ----- -----\n");

    for(w = 380; w <= 750.1; w += 1.0){
      ...
      printf("  %4.0f         %1.4f %1.4f %1.4f   ", w, x, y, z);
    ...
    for (i = 0, w = 380; w < 780.1; i++, w += 5) {
        x = cie_colour_match[i][0];
        y = cie_colour_match[i][1];
        z =  cie_colour_match[i][2];
    ...
    for(w = 380; w <= 760.1; w += 1.0){

      wave_to_xyz(w,&x,&y,&z);

      //printf(" :%3.0f { :lambda %3.0f :x %1.4f :y %1.4f :z %1.4f ", w, w, x, y, z);
      xyz_to_rgb(cs, x, y, z, &r, &g, &b);
      // printf("%.3f %.3f %.3f\n", r, g, b);
